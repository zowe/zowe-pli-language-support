...
`,type:Oe.MULTI_MODE_LEXER_LONGER_ALT_NOT_IN_CURRENT_MODE})}})}})})}return r}function kk(t,e,n){const r=[];let i=false;const a=ss(jt(ze(t.modes)));const s=tc(a,l=>l[Ar]===ot.NA);const o=kR(n);if(e){X(s,l=>{const u=AR(l,o);if(u!==false){const c=_k(l,u);const d={message:c,type:u.issue,tokenType:l};r.push(d)}else{if(Q(l,"LINE_BREAKS")){if(l.LINE_BREAKS===true){i=true}}else{if(Dp(o,l.PATTERN)){i=true}}}})}if(e&&!i){r.push({message:"Warning: No LINE_BREAKS Found.\n	This Lexer has been defined to track line and column information,\n	But none of the Token Types can be identified as matching a line terminator.\n	See https://chevrotain.io/docs/guide/resolving_lexer_errors.html#LINE_BREAKS \n	for details.",type:Oe.NO_LINE_BREAKS_FLAGS})}return r}function bk(t){const e={};const n=Vt(t);X(n,r=>{const i=t[r];if(le(i)){e[r]=[]}else{throw Error("non exhaustive match")}});return e}function SR(t){const e=t.PATTERN;if(Fn(e)){return false}else if(Hn(e)){return true}else if(Q(e,"exec")){return true}else if(wt(e)){return false}else{throw Error("non exhaustive match")}}function Nk(t){if(wt(t)&&t.length===1){return t.charCodeAt(0)}else{return false}}const Pk={test:function(t){const e=t.length;for(let n=this.lastIndex;n<e;n++){const r=t.charCodeAt(n);if(r===10){this.lastIndex=n+1;return true}else if(r===13){if(t.charCodeAt(n+1)===10){this.lastIndex=n+2}else{this.lastIndex=n+1}return true}}return false},lastIndex:0};function AR(t,e){if(Q(t,"LINE_BREAKS")){return false}else{if(Fn(t.PATTERN)){try{Dp(e,t.PATTERN)}catch(n){return{issue:Oe.IDENTIFY_TERMINATOR,errMsg:n.message}}return false}else if(wt(t.PATTERN)){return false}else if(SR(t)){return{issue:Oe.CUSTOM_LINE_BREAK}}else{throw Error("non exhaustive match")}}}function _k(t,e){if(e.issue===Oe.IDENTIFY_TERMINATOR){return`Warning: unable to identify line terminator usage in pattern.
...
DECLARE ${e.name} `;for(const i of n.attributes){r+=`${i.$cstNode?.text} `}r+="\n```";return r}else if(Ya(e)&&un(e.$container)){return this.getProcedureHoverContent(e.$container)}else if(un(e)){return this.getProcedureHoverContent(e)}else if(c$(e)){return"```\nDECLARE"+e.name+"\n```"}return""}getProcedureHoverContent(e){let n="```\n";for(const r of e.labels){n+=`${r.name} `}n+="PROCEDURE ";if(e.parameters.length>0){n+="("+e.parameters.map(r=>r.id).join(", ")+") "}if(e.recursive.length>0){n+="RECURSIVE "}if(e.order.includes("ORDER")){n+="ORDER "}else if(e.order.includes("REORDER")){n+="REORDER "}if(e.options.length>0){n+=e.options.map(r=>r.$cstNode?.text).join(" ")}if(e.returns.length>0){n+=e.returns.map(r=>r.$cstNode?.text).join(" ")}n+="\n```";return n}}class qO extends o${createReferenceCompletionItem(e){let n=void 0;if(e.type==="ProcedureStatement"){n="PROCEDURE"}else if(e.type==="DeclaredVariable"||e.type==="DoType3Variable"){n="DECLARE"}else if(e.type==="LabelPrefix"){n="LABEL"}const r=this.nodeKindProvider.getCompletionItemKind(e);const i=this.getReferenceDocumentation(e);return{nodeDescription:e,kind:r,documentation:i,detail:n,sortText:"0"}}}class jO extends Lv{isAffected(e,n){return false}}const BO=` // Mathematical functions
 ...
 END;
 `;class WO extends xv{factory;configStorage;constructor(e){super(e);this.factory=e.workspace.LangiumDocumentFactory;this.configStorage=e.workspace.PliConfigStorage}async loadAdditionalDocuments(e,n){const r=this.factory.fromString(BO,lt.parse("pli-builtin:///builtins.pli"));n(r);for(const i of e){const a=Ue.resolvePath(lt.parse(i.uri),"pgm_conf.json");try{const s=await this.fileSystemProvider.readFile(a);await this.configStorage.updateConfig(s,false)}catch{}}}traverseFolder(){return Promise.resolve()}}class VO extends l${didCloseDocument(e){const n=lt.parse(e.document.uri);if(n.scheme!=="pli-builtin"){this.fireDocumentUpdate([],[n])}}}class zO{documents;documentBuilder;compilerOptions;constructor(e){this.documentBuilder=e.workspace.DocumentBuilder;this.documents=e.workspace.LangiumDocuments;e.lsp.Connection?.onNotification("pli/pgmConfChanged",async n=>{await this.updateConfig(n.text)})}async updateConfig(e,n=true){try{const r=JSON.parse(e);this.compilerOptions=r;if(n){const i=this.documents.all.filter(a=>a.uri.scheme!=="pli-builtin").toArray();i.forEach(a=>{a.parseResult.value.$cstNode=void 0});await this.documentBuilder.update(i.map(a=>a.uri),[])}}catch(r){}}getCompilerOptions(e){return{options:this.compilerOptions??{},issues:[]}}}class XO extends Sv{createLangiumDocument(e,n,r,i){const a=this.serviceRegistry.getServices(n).parser.Lexer;a.uri=n;const s=super.createLangiumDocument(e,n,r,i);s.compilerOptions=a.compilerOptions;return s}async update(e,n){const r=this.serviceRegistry.getServices(e.uri).parser.Lexer;r.uri=e.uri;const i=await super.update(e,n);const a=i;a.compilerOptions=r.compilerOptions;return a}}const YO={documentation:{DocumentationProvider:t=>new HO(t)},validation:{Pl1Validator:()=>new EO,DocumentValidator:t=>new xO(t)},parser:{Lexer:t=>new OO(t),TokenBuilder:()=>new IO},references:{ScopeComputation:t=>new LO(t),NameProvider:()=>new FO,References:t=>new KO(t),ScopeProvider:t=>new UO(t)},lsp:{SemanticTokenProvider:t=>new MO(t),CompletionProvider:t=>new qO(t)}};const JO={lsp:{NodeKindProvider:()=>new GO,DocumentUpdateHandler:t=>new VO(t)},workspace:{LangiumDocumentFactory:t=>new XO(t),IndexManager:t=>new jO(t),WorkspaceManager:t=>new WO(t),PliConfigStorage:t=>new zO(t)}};function QO(t){const e=xu(zD(t),hO,JO);const n=xu(WD({shared:e}),yO,YO);e.ServiceRegistry.register(n);TO(n);if(!t.connection){e.workspace.ConfigurationProvider.initialized({})}return{shared:e,pli:n}}const ZO=new Zp.BrowserMessageReader(self);const eI=new Zp.BrowserMessageWriter(self);const tI=Zp.createConnection(ZO,eI);const{shared:nI}=QO({connection:tI,...Xv});hD(nI)});export default rI();
