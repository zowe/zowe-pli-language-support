...
describe("JSDoc parsing", () => {
  ...
  describe("Text and tags", () => {
    test("Can parse multiline text", () => {
      ...
      expectRange(lines[2].range, {
        start: { line: 2, character: 1 },
        end: { line: 2, character: 2 },
      });
      expect(lines[3]).toHaveProperty("text", "");
      expectRange(lines[3].range, {
  ...
  function expectRange(range: Range, expected: Range): void {
    expect(range.start.line, "Unexpected start line value").toBe(
      expected.start.line,
    );
    expect(range.start.character, "Unexpected start character value").toBe(
      expected.start.character,
    );
    expect(range.end.line, "Unexpected end line value").toBe(expected.end.line);
    expect(range.end.character, "Unexpected end character value").toBe(
...
describe("JSDoc rendering", () => {
  describe("Link rendering", () => {
    ...
    test("Renders URI as markdown linkcode #2", () => {
      const parsed = parseJSDoc("/** {@link https://langium.org/} */");
      expect(parsed.toMarkdown({ link: "code" })).toBe(
        "[`https://langium.org/`](https://langium.org/)",
      );
    });
