...
export class TestBuilder {
  ...
  expectExclusiveCompletions(label: string, completions: string[]) {
    const message = `Unexpected completions at label "${label}"`;

    this._expectCompletions(label, (completionResult) => {
      for (const completion of completions) {
        expect(completionResult, message).toContainEqual(completion);
      }

      expect(completionResult, message).toHaveLength(completions.length);
    });
  }
