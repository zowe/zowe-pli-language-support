...
import { DeclaredItem, Statement, SyntaxKind } from "../syntax-tree/ast";
...
class ProcedureSymbolBuilder implements SymbolBuilder {
  canHandle(token: IToken): boolean {
    return (
      token.tokenTypeIdx == CstNodeKind.EntryStatement_Variable &&
...
class DeclareSymbolBuilder implements SymbolBuilder {
  canHandle(token: IToken): boolean {
    if (
      token.tokenTypeIdx !== CstNodeKind.DefineOrdinalStatement_Unsigned0 ||
  ...
  buildSymbols(
    ...
    for (const item of token.payload?.element.items.filter(
...
function isValidToken(token: IToken): boolean {
  // Will fail for tokens with no end offset.
  // E.g., when generated by the preprocessor.
  return token.endOffset !== undefined && token.endOffset > 0;
}

class LabelSymbolBuilder implements SymbolBuilder {
  canHandle(token: IToken): boolean {
    if (
      token.tokenTypeIdx !== CstNodeKind.Reserves_Variables1 ||
