...
export interface CompositeTypeBuilder {
    flattenDeclareStatement(declareStatement: ast.DeclareStatement): BuilderDeclareItem[];
    isCompositeDeclaredItem(declaredItem: BuilderDeclareItem): boolean;
    handleCompositeDeclaredItem(declaredItem: BuilderDeclareItem, compilationUnit: CompilationUnit): TypeDescriptions.Structure;
    handlePrimitiveDeclaredItem(declaredItem: BuilderDeclareItem, compilationUnit: CompilationUnit): TypeDescriptions.Any;
}
...
export class DefaultCompositeTypeBuilder implements CompositeTypeBuilder {
    ...
    isCompositeDeclaredItem(declaredItem: BuilderDeclareItem): boolean {
        ...
        return declaredItem.level !== undefined && (declaredItem.attributes.length === 0 || declaredItem.attributes.every(isOnlyCompositeAttribute));
    }
    flattenDeclareStatement(declareStatement: ast.DeclareStatement): BuilderDeclareItem[] {
        return declareStatement.items.flatMap(item => this.flattenDeclaredItem(item));
    }
    private flattenDeclaredItem(declaredItem: ast.DeclaredItem): BuilderDeclareItem[] {
        ...
        for (const element of declaredItem.elements) {
            ...
            } else { //element.kind === ast.SyntaxKind.Wildcard
