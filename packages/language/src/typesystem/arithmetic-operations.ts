...
export type ComputeOperationReturnType = ({
  op,
  lhs,
  rhs,
}: {
...
export const createArithmeticOperationTable = (
  rulesOption: CompilerOptionRules,
) =>
  applyRules(
    [
      /**
       ...
       */
      whenAtLeastOneFloatCaseOnOperator(
        ["+", "-", "*", "/"],
        Base.Decimal,
        Base.Decimal,
        Base.Decimal,
      ...
      whenAtLeastOneFloatCaseOnOperator(
        ...
        Base.Binary,
        ({ p1, p2 }) =>
          Math.max(Math.ceil(p1 * DecimalToBinaryDigitsFactor), p2),
      ),
      ...
      whenAtLeastOneFloatCaseOnOperator(
        ...
        Base.Binary,
        ({ p1, p2 }) =>
          Math.max(p1, Math.ceil(p2 * DecimalToBinaryDigitsFactor)),
      ),
      ...
      rulesOption === CompilerOptionRules.ANS
        ? [
            /**
             ...
             */
            //## decimals only
            whenUnscaledFixedCaseOnOperatorANS(
              ["+", "-"],
              Base.Decimal,
              Base.Decimal,
              Base.Decimal,
              ({ p1, p2 }) => 1 + Math.max(p1, p2),
            ),
            whenUnscaledFixedCaseOnOperatorANS(
              ["*"],
              Base.Decimal,
            ...
            //## decimals only
            whenScaledFixedCaseOnOperatorANS(
              ["+", "-"],
              Base.Decimal,
              Base.Decimal,
              Base.Decimal,
              ({ p1, p2, q1, q2, q }) => 1 + Math.max(p1 - q1, p2 - q2) + q,
              ({ q1, q2 }) => Math.max(q1, q2),
            ),
            whenScaledFixedCaseOnOperatorANS(
              ["*"],
              Base.Decimal,
            ...
            whenScaledFixedCaseOnOperatorANS(
              ...
              Base.Binary,
              Base.Decimal,
              ({ p1, v, q1, q }) => 1 + Math.max(p1 - q1, v) + q,
              ({ q1 }) => q1,
            ),
            //ATTENTION! Broken IBM documentation here!
            whenScaledFixedCaseOnOperatorANS(
            ...
            whenScaledFixedCaseOnOperatorANS(
              ...
              Base.Decimal,
              Base.Decimal,
              ({ p2, w, q2, q }) => 1 + Math.max(w, p2 - q2) + q,
              ({ q2 }) => q2,
            ),
        ...
        : [
             ...
             */
            //## decimals only
            whenScaledFixedCaseOnOperatorIBM(
              ["+", "-"],
              Base.Decimal,
              Base.Decimal,
              Base.Decimal,
              ({ p1, q1, p2, q2, q }) => 1 + Math.max(p1 - q1, p2 - q2) + q,
              ({ q1, q2 }) => Math.max(q1, q2),
            ),
            whenScaledFixedCaseOnOperatorIBM(
              ["*"],
            ...
            whenScaledFixedCaseOnOperatorIBM(
              ...
              Base.Binary,
              Base.Binary,
              ({ p2, r, s, q, q2 }) => 1 + Math.max(r - s, p2 - q2) + q,
              ({ s, q2 }) => Math.max(s, q2),
            ),
            whenScaledFixedCaseOnOperatorIBM(
              ["*"],
            ...
            whenScaledFixedCaseOnOperatorIBM(
              ...
              Base.Decimal,
              Base.Binary,
              ({ p1, t, u, q, q1 }) => 1 + Math.max(p1 - q1, t - u) + q,
              ({ s, q1, u }) => Math.max(s, q1, u),
            ),
            whenScaledFixedCaseOnOperatorIBM(
              ["*"],
              Base.Binary,
...
function whenAtLeastOneFloatCaseOnOperator(
  whenOp: ArithmeticOperator[],
  whenLeftBase: Base,
  whenRightBase: Base,
  resultBase: Base,
  thenP: (variables: PComputationVariables) => number,
): ArithmeticTypeRule {
  return {
    whenOp,
    whenLeftBase,
 ...
 * @returns
 */
function whenUnscaledFixedCaseOnOperatorANS(
  whenOp: ArithmeticOperator[],
  whenLeftBase: Base,
  whenRightBase: Base,
  resultBase: Base,
  thenP: (variables: PComputationVariables) => number,
  thenQ?: (variables: QComputationVariables) => number,
): ArithmeticTypeRule {
  return {
    whenOp,
    whenLeftBase,
 ...
 * @returns
 */
function whenScaledFixedCaseOnOperatorANS(
  whenOp: ArithmeticOperator[],
  whenLeftBase: Base,
  whenRightBase: Base,
  resultBase: Base,
 ...
 * @returns
 */
function whenScaledFixedCaseOnOperatorIBM(
  whenOp: ArithmeticOperator[],
  whenLeftBase: Base,
  whenRightBase: Base,
  resultBase: Base,
...
function createVariablesForQ(
  ...
  return {
    ...
    get s() {
      return (
        Math.ceil(Math.abs(q1 * DecimalToBinaryDigitsFactor)) * Math.sign(q1)
      );
    },
    get t() {
    ...
    get u() {
      return (
        Math.ceil(Math.abs(q2 * DecimalToBinaryDigitsFactor)) * Math.sign(q2)
      );
    },
    get v() {
...
function createVariablesForP(
  variables: QComputationVariables,
  q: ((vars: QComputationVariables) => number) | undefined,
): PComputationVariables {
  return {
    ...variables,
    get q() {
