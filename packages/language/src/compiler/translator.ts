...
function ensureType(
  ...
  type: "plain",
): asserts value is AbstractCompilerOptionText;
function ensureType(
  value: AbstractCompilerValue,
  type: "string",
): asserts value is AbstractCompilerOptionString;
function ensureType(
  value: AbstractCompilerValue,
  type: "plainOrString",
): asserts value is AbstractCompilerOptionString | AbstractCompilerOptionText;
function ensureType(
  value: AbstractCompilerValue,
  type: "option",
...
function ensureType(value: AbstractCompilerValue, type: string): void {
  ...
  if (type !== received) {
    throw new TranslationError(
      value.token,
      `Expected a ${type}, received ${received}.`,
      1,
    );
  }
}
...
function stringTranslate(
  callback: (
    options: CompilerOptions,
    value: AbstractCompilerOptionString,
  ) => void,
): Translate {
...
function plainTranslate(
  callback: (
    options: CompilerOptions,
    value: AbstractCompilerOptionText,
  ) => void,
  ...values: string[]
): Translate {
  return (option, options) => {
...
translator.rule(["DISPLAY"], (option, options) => {
  ...
  if (value.type === "text") {
    ...
    throw new TranslationError(
      value.token,
      `Invalid display value. Expected a text or an option, but received '${value.type}'.`,
      1,
    );
  }
});
