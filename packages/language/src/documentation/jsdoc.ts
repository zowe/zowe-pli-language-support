...
export function isJSDocParagraph(value: JSDocValue): value is JSDocParagraph {
    return 'inlines' in value;
}
 ...
 */
export function parseJSDoc(token: Token, options?: JSDocParseOptions): JSDocComment;
/**
...
export function parseJSDoc(node: Token | string, start?: Position | JSDocParseOptions, options?: JSDocParseOptions): JSDocComment {
...
export function isJSDoc(node: Token | string, options?: JSDocParseOptions): boolean {
...
function getLines(node: Token | string): string[] {
    let content = '';
    if (typeof node === 'string') {
        content = node;
    } else {
