...
function parseJSDocComment(context: ParseContext): JSDocComment {
  const startPosition: Position = Position.create(
...
function parseJSDocElement(
  context: ParseContext,
  last?: JSDocElement,
): JSDocElement | undefined {
...
function normalizeOption(
  ...
  if (typeof option === "string" || typeof option === "object") {
    const escaped =
      typeof option === "string" ? escapeRegExp(option) : option.source;
    if (start) {
...
function renderInlineTag(
  ...
  if (tag === "linkplain" || tag === "linkcode" || tag === "link") {
    ...
    const renderedLink =
      options.renderLink?.(content, display) ??
