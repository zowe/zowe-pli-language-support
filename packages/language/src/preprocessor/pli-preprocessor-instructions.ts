...
export interface PPIBranchIfNotEqual extends PPInstructionBase {
  type: "branchIfNEQ";
  address: ProgramAddressOrPlaceholder;
  evalFn: (b: boolean | undefined) => void;
}
...
export namespace Values {
  ...
  function numericComparisonOp(op: (lhs: number, rhs: number) => boolean) {
    return function func(lhs: IToken[], rhs: IToken[]) {
      ...
      if (
        rhs.length !== 1 ||
        !sameType(rhs[0].tokenType, PreprocessorTokens.Number)
      ) {
        return [];
      }
    ...
    switch (instruction.type) {
      ...
      case "push":
        programText.push(
          ...
          "[",
          instruction.value
            .map((tk) => tk.image + ":" + tk.tokenType.name)
            .join(", "),
          "]",
        );
        break;
...
export function printInstruction(
  instruction: PPInstruction,
  state: PreprocessorInterpreterState,
) {
  const programText: string[] = [];
  programText.push(instruction.type.toUpperCase());
  switch (instruction.type) {
