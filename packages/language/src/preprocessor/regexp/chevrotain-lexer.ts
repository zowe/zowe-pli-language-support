...
export function analyzeTokenTypes(
  ...
  tracer("Misc Mapping #2", () => {
    ...
    patternIdxToConfig = map(
      ...
      (x, idx): TokenType & IPatternConfig => {
        return {
          ...onlyRelevantTypes[idx],
  ...
  let unoptimizedPatterns: TokenType[] = [];
  ...
  if (!options.safeMode) {
    tracer("First Char Optimization", () => {
      charCodeToPatternIdxToConfig = reduce(
        ...
        [] as { [charCode: number]: (TokenType & IPatternConfig)[] },
      );
    });
  }
...
export function findEndOfInputAnchor(
  ...
  class EndAnchorFinder extends BaseRegExpVisitor {
    ...
    override visitEndAnchor(node: unknown) {
      this.found = true;
    }
  }
...
export function findStartOfInputAnchor(
  ...
  class StartAnchorFinder extends BaseRegExpVisitor {
    ...
    override visitStartAnchor(node: unknown) {
      this.found = true;
    }
  }
