...
// Bound for generated tokens, i.e. those not originally in the input text), such as
// %ANSWER statements, or token replacements.
const MAX_EMITTED_TOKENS = 1_000_000;

// Cumulative bound on all variables. Otherwise, we can go out of memory on large arrays
// with large strings.
const MAX_TOTAL_VALUE_LENGTH = 10_000_000;
...
/**
 * Mutable interpreter state responsible for keeping track of executed instructions.
 * Ensures that we never run out of memory or get stuck on some instructions.
 */
interface RunState {
  /**
   * Tokens generated by the run so far. The first over-budget emission pins it to
   * `MAX_EMITTED_TOKENS + 1`, so all later {@link acceptEmittedTokens} calls fail.
   */
  emittedTokens: number;
  /** Characters currently retained in preprocessor variables (see {@link MAX_TOTAL_VALUE_LENGTH}). */
  storedChars: number;
  /** Whether the {@link LspCodes.StorageTooLarge} diagnostic was already reported for this run. */
  storageExhaustedReported: boolean;
  /**
   * Whether the node currently being dispatched is a re-execution. Set by
   * {@link chargeInstruction}; source-token re-emissions are only charged when set.
   */
  replayedNode: boolean;
  /**
   * The preprocessor cannot recover, flag to abort the preprocessor
   */
  aborted: boolean;
...
export async function runInstructions(
  ...
  const context: InterpreterContext = {
    ...
    state: {
      emittedTokens: 0,
      storedChars: 0,
      storageExhaustedReported: false,
      replayedNode: false,
      aborted: false,
...
function chargeInstruction(
  ...
  if (
    ...
    // This run's output is already obsolete; unwind through the whole pipeline so it is
    // never serialized or re-lexed.
    throw OperationCancelled;
  }
  const replayed = state.executedNodes.has(node);
  state.replayedNode = replayed;
  if (replayed) {
    if (
      ...
      if (!state.budgetExhaustedReported) {
        ...
        context.diagnostics.push(
          diagnosticFromCodeAtRange(
            LspCodes.BudgetExhausted,
            context.entryUri.toString(),
            { start: 0, end: 0 },
          ),
        );
      }
...
async function doRunInstructions(
  ...
  const cancellation = context.options.cancellation;
  ...
  while (currentNode && !context.state.aborted) {
    ...
    if (
      cancellation &&
      context.state.dispatchedInstructions % CANCELLATION_CHECK_INTERVAL === 0
    ) {
      // Without this yield the `$/cancelRequest` message is never processed during a
      // CPU-bound run, so the poll above could not observe an in-flight cancellation.
...
function handleInstructionError(
  ...
  if (isOperationCancelled(err)) {
    // Not an instruction failure - keep unwinding.
    throw err;
  }
...
function runAnswerInstruction(
  ...
  if (instruction.expression) {
    // Always evaluated for its side effects (e.g. COUNTER, procedure calls); once the
    // budget is exhausted, lexing and rescanning the value would be pure dead work.
    const expression = evaluateExpression(instruction.expression, context);
    if (!emissionBudgetExhausted(context.state)) {
      ...
      if (acceptEmittedTokens(tokens.length, context)) {
        if (breakCount > 0) {
 ...
 */
function acceptEmittedTokens(
  count: number,
  context: InterpreterContext,
): boolean {
  const state = context.state;
  if (state.emittedTokens + count > MAX_EMITTED_TOKENS) {
    if (!emissionBudgetExhausted(state)) {
      // Only report the first time the budget is exceeded
      state.emittedTokens = MAX_EMITTED_TOKENS + 1;
      console.log(
        `Preprocessor tried to emit more than ${MAX_EMITTED_TOKENS} tokens. Dropping further output.`,
      );
      context.diagnostics.push(
        diagnosticFromCodeAtRange(
          LspCodes.OutputTruncated,
          context.entryUri.toString(),
          { start: 0, end: 0 },
          MAX_EMITTED_TOKENS,
        ),
  ...
  state.emittedTokens += count;
  return true;
}

/** `true` once {@link acceptEmittedTokens} has rejected an emission (permanent). */
function emissionBudgetExhausted(state: RunState): boolean {
  return state.emittedTokens > MAX_EMITTED_TOKENS;
}
  ...
  for (const ref of instruction.refs) {
    ...
    if (!variable) {
      ...
      variable = {
        ...
        value: chargeStorage(context, undefined, value, ref.reference?.token)
          ? value
          : defaultEmptyValue,
...
/** Characters retained by a value; array elements count at least one each. */
function valueSize(value: Value): number {
  if (isArrayValue(value)) {
    let size = value.array.length;
    for (const element of value.array) {
...
function chargeStorage(
  ...
  const state = context.state;
  const delta = valueSize(newValue) - (oldValue ? valueSize(oldValue) : 0);
  if (delta > 0 && state.storedChars + delta > MAX_TOTAL_VALUE_LENGTH) {
    if (!state.storageExhaustedReported) {
      state.storageExhaustedReported = true;
      const diagnostic = token
        ? diagnosticFromCode(
            LspCodes.StorageTooLarge,
            token,
...
function evaluateValueAccess(
  ...
  args: inst.ExpressionInstruction[],
  context: InterpreterContext,
  /** Anchor for diagnostics reported by the store, if the source has one. */
  token?: Token,
): ValueAccess {
  ...
  if (args.length === 0) {
    ...
    return {
      ...
      setter: (val) => {
        if (chargeStorage(context, variable.value, val, token)) {
          variable.value = val;
        }
      },
    };
      ...
      if (i === args.length - 1) {
        if (isArrayValue(indexedValue)) {
          ...
          return {
            getter: () => indexedValue,
            setter: (val) => {
              if (chargeStorage(context, indexedValue, val, token)) {
...
function runTokenInstruction(
  ...
  // First-execution passthrough is free (the program must reach the parser however
  // large it is); a replayed node re-emits the same tokens on every pass, so its output
  // is charged against the shared budget.
  const replay = context.state.replayedNode;
  if (replay && emissionBudgetExhausted(context.state)) {
    return;
  }
  ...
  while (i < length) {
    ...
    if (result) {
      // Replace the token with the scan result, i.e. the variable content, recursively
      // replaced (already charged against the budget by `replaceTokenWithValue`)
      if (!emissionBudgetExhausted(context.state)) {
      ...
      if (replay && !acceptEmittedTokens(1, context)) {
        return;
      }
      if (i === 0) {
...
function replaceTokenWithValue(
  ...
  // Chained active variables expand exponentially within a single dispatch, where the
  // instruction budget cannot intervene, so each rescan level is charged against the
  // output budget; once saturated, the value is returned unexpanded.
  if (!acceptEmittedTokens(tokens.length, context)) {
    return tokens;
  }
