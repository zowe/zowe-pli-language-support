...
describe("CICS SEND", async () => {
  ...
  test("Positive (group1)", async () => {
    const { diagnostics } = await cicsPreprocessor.execute("SEND FROM(VAR)");
    expect(diagnostics).toHaveLength(0);
  });

  test("Expecting EOF", async () => {
    const { diagnostics } = await cicsPreprocessor.execute("SEND FROM(VAR) BLA");
  ...
  test("TEXT missing FROM", async () => {
    const { diagnostics } = await cicsPreprocessor.execute("SEND TEXT");
    expect(diagnostics).toHaveLength(1);
    expect(diagnostics[0].severity).toBe(Severity.Error);
  ...
  test("TEXT MAPPED missing FROM", async () => {
    const { diagnostics } = await cicsPreprocessor.execute("SEND TEXT MAPPED");
    expect(diagnostics).toHaveLength(1);
    expect(diagnostics[0].severity).toBe(Severity.Error);
    expect(diagnostics[0].message).toMatch(/Missing required option: FROM/);
  });
