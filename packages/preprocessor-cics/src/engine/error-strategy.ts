...
import { DefaultErrorStrategy, FailedPredicateException, InputMismatchException, IntervalSet, NoViableAltException, Parser, RecognitionException, Token } from "antlr4ng";
...
export class CICSErrorStrategy extends DefaultErrorStrategy {
  ...
  private static readonly BLOCK_END_TOKENS = new IntervalSet([CICSLexer.END_EXEC, CICSLexer.DOT]);
  ...
  private  reportUnrecognizedException(recognizer: Parser, e: RecognitionException) {
  ...
  override reportInputMismatch(recognizer: Parser, e: InputMismatchException) {
    const msg = this.getInputMismatchMessage(recognizer, e, e.offendingToken!, this.getOffendingToken(e));
  ...
  override reportNoViableAlternative(recognizer: Parser, e: NoViableAltException) {
    const input = recognizer.inputStream.getTextFromRange(e.startToken, e.offendingToken)
  ...
  override reportMissingToken(recognizer: Parser) {
    ...
    const newMessage = this.buildErrorMessage(this.removeIdentifierTokens(this.collectErrorTokens(recognizer, expectedTokens)));
  ...
  private collectErrorTokens(recognizer: Parser, interval: IntervalSet): string[] {
    ...
    const MSG_PREFIX = "{";
    const MSG_SUFFIX = "}";
    return interval.toStringWithVocabulary(recognizer.vocabulary)
                .replace(MSG_PREFIX, "")
                .replace(MSG_SUFFIX, "")
                .split(MSG_DELIMITER);
  }
  ...
  private getInputMismatchMessage(
      recognizer: Parser, e: InputMismatchException, token: Token, offendingTokens: string) {
  ...
  private getOffendingToken(e: InputMismatchException | NoViableAltException): string {
    return this.getTokenErrorDisplay(e.offendingToken);
  }

  private getExpectedText(recognizer: Parser, e: InputMismatchException): string {
    const expectedTokens = recognizer.getExpectedTokens();
    const newMessage = this.buildErrorMessage(this.removeIdentifierTokens(this.collectErrorTokens(recognizer, expectedTokens)));
