...
export interface MessageService {
  getMessage<K extends keyof typeof EN>(
    key: K,
    ...parameters: Parameters<(typeof EN)[K]>
  ): string;
}
...
const EN = {
  ...
  "CobolLineReaderImpl.longLineMsg": (col: number) =>
    `Source text cannot go past column ${col}`,
  ...
  "CobolVisitor.AreaAWarningMsg": (token: string) =>
    `The following token must start in Area A: ${token}`,
  "CobolVisitor.AreaBWarningMsg": (token: string) =>
    `The following token must start in Area B: ${token}`,
  ...
  "CobolVisitor.misspelledWord": (word: string) =>
    `A misspelled word, maybe you want to put ${word}`,
  ...
  "CobolVisitor.subroutineNotFound": (subroutine: string) =>
    `${subroutine}: Subroutine not found`,
  "semantics.functionExpected": (functionName: string) =>
    `Expected a function name, but found '${functionName}'`,
  "semantics.functionRedefined": (functionName: string) =>
    `Function '${functionName}' redefined`,
  ...
  "semantics.noFileControl": (item: string) =>
    `No FILE-CONTROL entry found for ${item}`,
  ...
  "Communications.noSyntaxError": (file: string) =>
    `No syntax errors detected in ${file}`,
  "Communications.syntaxAnalysisInProgress": (file: string) =>
    `${file} : Syntax analysis in progress`,
  "Communications.syntaxAnalysisInProgressTitle": (file: string) =>
    `Parsing ${file}`,
  ...
  "missing.period": (token: string) =>
    `A period was assumed before "${token}".`,
  ...
  "db2SqlParser.currentQueryAcceleration": (query: string) =>
    `${query} not valid. Must be of format %d%d%d%d.%d`,
  "db2SqlParser.maxIntValue": (value: string) =>
    `${value} not allowed. Allowed range is 1 to 32767`,
  "db2SqlParser.pieceSize": (size: string) =>
    `${size} not allowed. Size must be in KB, MB or GB`,
  "db2SqlParser.size": (size: string) =>
    `${size} not allowed. Size must be in GB`,
  ...
  "parsers.intRangeValue": (min: string, max: string) =>
    `Allowed range is ${min} to ${max}`,
  "parsers.alphaNumeric": (value: string) =>
    `Only alphanumerics are allowed for ${value}`,
  "parsers.startsWith": (value: string) =>
    `String must starts with ${value} values`,
  "parsers.stringLengthRange": (min: string, max: string) =>
    `String length must be between ${min} and ${max}`,
  "parsers.allowedStringValues": (values: string) =>
    `Only allowed value(s): ${values}`,
  "parsers.notAllowedVariableName": (name: string) =>
    `Variable name ${name} is not allowed`,
  ...
  "ErrorStrategy.reportNoViableAlternative": (input: string) =>
    `No viable alternative at input ${input}`,
  "ErrorStrategy.reportInputMismatch": (input: string) =>
    `Syntax error on ${input}`,
  "ErrorStrategy.reportUnwantedToken": (input: string) =>
    `Extraneous input ${input}`,
  "ErrorStrategy.performMissingEnd": (input: string) =>
    `Extraneous input ${input} expected END-PERFORM`,
  ...
  "GrammarPreprocessorListener.errorSuggestion": (copybook: string) =>
    `${copybook}: Copybook not found`,
  "GrammarPreprocessorListener.recursionDetected": (copybook: string) =>
    `Recursive copybook declaration for: ${copybook}`,
  ...
  "GrammarPreprocessorListener.controlDirectiveWrongArgs": (
    directive: string,
  ) => `No arguments found for *${directive}`,
  ...
  "semantics.cannotBeRenamed": (dataEntry: string) =>
    `The data entry ${dataEntry} cannot be renamed`,
  ...
  "semantics.notDefined": (variable: string) =>
  ...
  "statements.invalidSendingField": (expected: string) =>
    `Invalid sending field type. Expected: ${expected}`,
  "semantics.picAndUsageConflict": () =>
    "PICTURE and USAGE clause are not compatible",
  "semantics.noPicClause": (usage: string) =>
    `PICTURE clause incompatible with usage ${usage}`,
  "semantics.improperUseBlankWhenZeroAndSignClause": (usage: string) =>
    `${usage} not compatible with USAGE clause`,
  ...
  "variables.elementaryWithType": (type: string) =>
    `Elementary ${type} data item`,
  ...
  "jsonParseProcess.condition2": (item: string) =>
    `${item} must be a level-88.`,
  ...
  "jsonParseProcess.omittedIdentifier3": (item1: string, item2: string) =>
    `${item1} must reference ${item2}.`,
  ...
  "xmlParse.identifier2.dataType": (item: string) =>
    `${item} must be of category alphanumeric`,
  ...
  "compilerDirective.deprecatedDirectiveUse": (directive: string) =>
    `${directive} is a deprecated compiler directive`,
  ...
  "compilerDirective.info.deprecatedDirectiveUse": (option: string) =>
    `The "${option}" option is no longer supported.`,
  ...
  "procedureDivisionHeaderProcess.wrongDataName": (dataName: string) =>
    `'${dataName}' was not defined as a data-name`,
