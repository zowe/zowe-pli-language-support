...
export interface MessageService {
  getMessage<K extends keyof typeof EN>(key: K, ...parameters: Parameters<typeof EN[K]>): string;
}

export class EnglishMessageService implements MessageService {
  getMessage<K extends keyof typeof EN>(key: K, ...parameters: Parameters<typeof EN[K]>): string {
    try {
      return (EN[key] as any)(...parameters) ?? key;
    } catch {
      return key;
    }
  }
...
const EN = {
  ...
  "CobolLineReaderImpl.longLineMsg": (col: number) => `Source text cannot go past column ${col}`,
  "cobolParser.subSchemaNameLength": (length: number) =>
  ...
  "CobolVisitor.AreaAWarningMsg": (token: string) => `The following token must start in Area A: ${token}`,
  "CobolVisitor.AreaBWarningMsg": (token: string) => `The following token must start in Area B: ${token}`,
  "CobolVisitor.declarativeSameMsg": (token: string) =>
    `The following token cannot be on the same line as a DECLARATIVE token: ${token}`,
  "CobolVisitor.identicalProgMsg": (token: string) =>
    `Program-name must be identical to the program-name of the corresponding PROGRAM-ID paragraph: ${token}`,
  "CobolVisitor.functionReturningMsg": () => "Procedure division of a function must contain RETURNING clause",
  "CobolVisitor.misspelledWord": (word: string) => `A misspelled word, maybe you want to put ${word}`,
  "CobolVisitor.duplicateFileName": (fileName: string) =>
  ...
  "CobolVisitor.funcIDIssueMsg": () => "There is an issue with FUNCTION-ID paragraph",
  "CobolVisitor.subroutineNotFound": (subroutine: string) => `${subroutine}: Subroutine not found`,
  "semantics.functionExpected": (functionName: string) =>
    `Expected a function name, but found '${functionName}'`,
  "semantics.functionRedefined": (functionName: string) => `Function '${functionName}' redefined`,
  "semantics.redefinedContainValue": (item: string) =>
    `The redefining item cannot contain a VALUE clause: ${item}`,
  "semantics.redefineImmediatelyFollow": (item: string) =>
    `REDEFINES line must immediately follow redefined item: ${item}`,
  "semantics.levelsMustMatch": (item: string) =>
    `The redefining and redefined items must have the same level: ${item}`,
  "semantics.noFileControl": (item: string) => `No FILE-CONTROL entry found for ${item}`,
  "cics.enableTranslator": () => "Enable CICS translator in the settings",
  "semantics.unknownVariableDefinition": () => "Unknown variable definition",
  "Communications.noSyntaxError": (file: string) => `No syntax errors detected in ${file}`,
  "Communications.syntaxAnalysisInProgress": (file: string) => `${file} : Syntax analysis in progress`,
  "Communications.syntaxAnalysisInProgressTitle": (file: string) => `Parsing ${file}`,
  ...
  "missing.period": (token: string) => `A period was assumed before "${token}".`,
  ...
  "db2SqlParser.currentQueryAcceleration": (query: string) => `${query} not valid. Must be of format %d%d%d%d.%d`,
  "db2SqlParser.maxIntValue": (value: string) => `${value} not allowed. Allowed range is 1 to 32767`,
  "db2SqlParser.pieceSize": (size: string) => `${size} not allowed. Size must be in KB, MB or GB`,
  "db2SqlParser.size": (size: string) => `${size} not allowed. Size must be in GB`,
  "parsers.validValueMsg": (value: string, expected: string) =>
    `${value} not allowed. It must be ${expected}`,
  "parsers.maxLength": (value: string, max: string) =>
    `${value} cannot exceed ${max} characters`,
  "parsers.exactLength": (value: string, length: string) =>
    `Exact length of ${value} must be ${length} characters`,
  "parsers.intRangeValue": (min: string, max: string) => `Allowed range is ${min} to ${max}`,
  "parsers.alphaNumeric": (value: string) => `Only alphanumerics are allowed for ${value}`,
  "parsers.startsWith": (value: string) => `String must starts with ${value} values`,
  "parsers.stringLengthRange": (min: string, max: string) =>
    `String length must be between ${min} and ${max}`,
  "parsers.allowedStringValues": (values: string) => `Only allowed value(s): ${values}`,
  "parsers.notAllowedVariableName": (name: string) => `Variable name ${name} is not allowed`,
  ...
  "ErrorStrategy.reportMissingToken": (token: string, location: string) =>
    `Missing token ${token} at ${location}`,
  "ErrorStrategy.reportNoViableAlternative": (input: string) => `No viable alternative at input ${input}`,
  "ErrorStrategy.reportInputMismatch": (input: string) => `Syntax error on ${input}`,
  "ErrorStrategy.reportUnwantedToken": (input: string) => `Extraneous input ${input}`,
  "ErrorStrategy.performMissingEnd": (input: string) => `Extraneous input ${input} expected END-PERFORM`,
  "GrammarPreprocessorListener.copyBkContainsUnderScore": (copybook: string) =>
    `Copybook declaration has '_' characters for: ${copybook}`,
  "GrammarPreprocessorListener.copyBkOverMaxChars": (max: number, copybook: string) =>
    `Copybook declaration has more than ${max} characters for: ${copybook}`,
  "GrammarPreprocessorListener.copyBkStartsOrEndsWithHyphen": (copybook: string) =>
    `Copybook declaration starts or ends with '-' characters for: ${copybook}`,
  "GrammarPreprocessorListener.errorSuggestion": (copybook: string) => `${copybook}: Copybook not found`,
  "GrammarPreprocessorListener.recursionDetected": (copybook: string) => `Recursive copybook declaration for: ${copybook}`,
  "GrammarPreprocessorListener.copyBkNestedReplaceStmt": (copybook: string) =>
    `More than one nested copy replace statement for copybook declaration of: ${copybook}`,
  "GrammarPreprocessorListener.controlDirectiveWrongArgs": (directive: string) => `No arguments found for *${directive}`,
  "GrammarPreprocessorListener.langMissingEnterDirective": () => "Language name missing for ENTER compiler directive",
  "ReplacingServiceImpl.invalidWord": () => "COPY string is not allowed in pseudo text",
  "ReplacingServiceImpl.pseudoTxtInvalidLength": () => "Max 322 chars allowed for each individual word in pseudo text",
  "semantics.paragraphNotDefined": (paragraph: string) =>
    `The following paragraph is not defined: ${paragraph}`,
  "semantics.cannotBeRenamed": (dataEntry: string) => `The data entry ${dataEntry} cannot be renamed`,
  "semantics.childToRenameNotFound": (dataEntry: string) =>
    `The data entry with the name ${dataEntry} not found`,
  "semantics.emptyStructure": (dataEntry: string) =>
  ...
  "semantics.notDefined": (variable: string) => `Variable ${variable} is not defined`,
  "semantics.notDefinedInStructure": (variable: string, structure: string) =>
    `Variable ${variable} does not exist in structure ${structure}`,
  "semantics.ambiguous": (reference: string) => `Ambiguous reference for ${reference}`,
  "semantic.exitPerformIsIgnored": () => "The EXIT PERFORM statement is outside of an inline PERFORM statement and will be ignored",
  "semantic.picNotAllowed": (variable: string) => `Group variable ${variable} cannot have PICTURE clause`,
  "statements.invalidReceivingField": (expected: string) =>
    `Invalid receiving field type. Expected: ${expected}`,
  "statements.invalidSendingField": (expected: string) => `Invalid sending field type. Expected: ${expected}`,
  "semantics.picAndUsageConflict": () => "PICTURE and USAGE clause are not compatible",
  "semantics.noPicClause": (usage: string) => `PICTURE clause incompatible with usage ${usage}`,
  "semantics.improperUseBlankWhenZeroAndSignClause": (usage: string) => `${usage} not compatible with USAGE clause`,
  ...
  "dialects.missingDialect": (dialect: string, requiredFor: string) =>
    `${dialect} dialect is missing (required for ${requiredFor})`,
  "dialects.processingError": (dialect: string, requiredFor: string) =>
  ...
  "xmlParse.identifier2.dataType": (item: string) => `${item} must be of category alphanumeric`,
  ...
  "deleteFileOperation.notOpened": () => "DELETE statement can only reference a file opened for I-O",
  "startFileOperation.notOpened": () => "START statement can only reference a file opened for INPUT or I-O",
  "compilerDirective.deprecatedDirectiveUse": (directive: string) => `${directive} is a deprecated compiler directive`,
  "compilerDirective.warning.deprecatedDirectiveUse": (option: string) =>
    `The "${option}" compiler option is not supported. The option was ignored.`,
  "compilerDirective.info.deprecatedDirectiveUse": (option: string) => `The "${option}" option is no longer supported.`,
  ...
  "compilerDirective.javaShareable.On": () => "The JAVA-SHAREABLE state was already ON.",
  "compilerDirective.javaShareable.Off": () => "The JAVA-SHAREABLE state was already OFF.",
  ...
  "db2Parser.missingSql": () => "Missing token SQL for the EXEC SQL INCLUDE block ",
  ...
  "db2Parser.missingEndExec": () => "Missing token END-EXEC for the EXEC block",
  "cobolParser.expectSpace": () => "A blank is missing after ','",
  "procedureDivisionHeaderProcess.wrongNodeLocation": (operand: string) =>
    `Operand '${operand}' was not defined in the LINKAGE SECTION`,
  "procedureDivisionHeaderProcess.wrongDataName": (dataName: string) => `'${dataName}' was not defined as a data-name`,
  "variableNameCheck.notAllowedVariableName": (name: string) =>
    `The name ${name} cannot be used as a data-name because it was already declared as a function`,
  "cics.invalidLiteralDelimeter": (delimiter: string) =>
  ...
  "cobolParser.unknownExecBlockUnterminated": () => "Unknown EXEC block is not terminated",
};