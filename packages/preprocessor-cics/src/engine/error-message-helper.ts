...
export class ErrorMessageHelper {
  ...
  private static readonly REPORT_INPUT_MISMATCH =
    "ErrorStrategy.reportInputMismatch";
  private static readonly REPORT_UNWANTED_TOKEN =
   ...
   * @return error message string
   */
  public getUnwantedTokenMessage(
    recognizer: Parser,
    currentToken: Token,
  ): string {
  ...
  public getExpectedText(recognizer: Parser): string {
    return this.getExpectedTextByInterval(
      recognizer,
   ...
   * @return input string
   */
  public retrieveInputForNoViableException(
    recognizer: Parser,
  ...
  private getExpectedTextByException(
    recognizer: Parser,
    e: InputMismatchException,
  ): string {
  ...
  private getExpectedTextByInterval(
    recognizer: Parser,
    interval: IntervalSet,
  ): string {
    const newMessage = this.buildErrorMessage(
      this.removeIdentifierTokens(
        this.collectErrorTokens(recognizer, interval),
      ),
    );
