...
describe("CICS INQUIRE (system programming)", async () => {
  ...
  test("STORAGE without ADDRESS or NUMELEMENTS", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE STORAGE");
    expect(diagnostics).toHaveLength(1);
  ...
  test("bundle", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE BUNDLE(BN)");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("enq next", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE ENQ NEXT");
    expect(diagnostics).toHaveLength(0);
  });
  test("enq end", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE ENQ END");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("statistics", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE STATISTICS");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("mvstcb next", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE MVSTCB NEXT");
    expect(diagnostics).toHaveLength(1);
  });
  ...
  test("netname", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE NETNAME(NN)");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("program", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE PROGRAM(PG)");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("program end", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE PROGRAM END");
    expect(diagnostics).toHaveLength(0);
  });
  test("reqid", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE REQID(RQ)");
    expect(diagnostics).toHaveLength(0);
  });
  test("reqid browse", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE REQID START");
    expect(diagnostics).toHaveLength(0);
  });
  test("storage", async () => {
    const { diagnostics } = await cicsPreprocessor.parse(
  ...
  test("subpool", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE SUBPOOL(SP)");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("tag next", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE TAG NEXT");
    expect(diagnostics).toHaveLength(1);
  });
  test("tag start", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE TAG START");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("tsqueue", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE TSQUEUE(TQ)");
    expect(diagnostics).toHaveLength(0);
  });
  ...
  test("uowenq next", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE UOWENQ NEXT");
    expect(diagnostics).toHaveLength(0);
  });
  test("uowenq end", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE UOWENQ END");
    expect(diagnostics).toHaveLength(0);
  });
  test("vtam", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE VTAM");
    expect(diagnostics).toHaveLength(0);
  });
  test("common (FILE)", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("INQUIRE FILE(FF)");
    expect(diagnostics).toHaveLength(0);
  });
