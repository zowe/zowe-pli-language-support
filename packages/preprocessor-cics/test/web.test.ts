...
describe("CICS WEB", async () => {
  ...
  test("RETRIEVE missing DOCTOKEN", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("WEB RETRIEVE");
    expect(diagnostics).toHaveLength(1);
  ...
  test("CLOSE missing SESSTOKEN", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("WEB CLOSE");
    expect(diagnostics).toHaveLength(1);
  });
  ...
  test("PARSE", async () => {
    const { diagnostics } = await cicsPreprocessor.parse("WEB PARSE URL(UL)");
    expect(diagnostics).toHaveLength(0);
  });
