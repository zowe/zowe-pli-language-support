...
async function main() {
  ...
  for (const [enumName, literals] of Object.entries(enums)) {
    for (const literal of literals) {
      ...
      const location = hasLocation
        ? `[tokens.ts#${refMap.get(key)}](${TokensFileName}#L${refMap.get(key)})`
        : "";
  ...
  if (hasMissing) {
    ...
    console.error(
      `Missing token-to-enum mappings found. See ${ReportFileName} for details.`,
    );
    process.exit(1);
  } else {
    console.log("All token-to-enum mappings are present. No report generated.");
  }
}
