...
`chargeInstruction` then guards every dispatched instruction: the *first* execution of an instruction node is always allowed (a program, including everything it `%INCLUDE`s, must be interpreted end to end), while every *re-execution* - a further loop iteration, `%GOTO` cycle, procedure call or repeated include - is charged against one budget shared by the whole run (`limit * GLOBAL_INSTRUCTION_FACTOR`).
...
Exhaustion stops the current instruction chain, so instructions after an exhausted loop in the same chain (including the plain source text they carry) are dropped; the truncation is reported as an editor-visible diagnostic (`LSPTL005`).
The same function polls the build's `CancellationToken` every `CANCELLATION_CHECK_INTERVAL` (`1000`) instructions and throws `OperationCancelled` when it fires, so an in-flight run is given up when the document changes or the request is withdrawn; the async instruction loop yields to the event loop at the same interval (via `interruptAndCheck`), which is what allows the cancellation message to be processed during a CPU-bound run at all. The pipeline additionally checks the token between phases and the EXEC phases check it once per processed file.
Additional guards: circular `next`-chain detection, a `MAX_ARRAY_COUNT` (100 000) cap on array sizes, a `MAX_VALUE_LENGTH` (100 000) cap on `||` concatenation results and `COPY`/`REPEAT` output, a `MAX_EMITTED_TOKENS` (1 000 000) cap on the tokens a run may *generate* (`%ANSWER` output, rescan expansions, and re-emissions of source tokens replayed by a loop or `%GOTO`; the first overflow permanently drops all further generated output and reports `LSPTL004`), a `MAX_TOTAL_VALUE_LENGTH` (10 000 000 characters) cap on the aggregate storage retained by preprocessor variables (`LSPTL003`), aborting the instruction loops on `RangeError` (except stack overflows, which are localized; the abort reports `LSPTL006`), the `COUNTER` builtin wrapping at 99999, and a workaround for V8's spread-argument limit when emitting very large token arrays.

## The SQL and CICS phases

The `EXEC` phases mirror how the real precompilers work: the engine receives the phase's **full text** and finds everything it owns itself - its `EXEC SQL ...;`/`EXEC CICS ...;` statements as well as its host-side built-ins - rather than being handed pre-cut fragments. The phase never tokenizes: the only host-side work is turning the recorded edits into LSP metadata.

- **Scanning**: [`scanHostText`](../packages/preprocessor-api/src/context-utils.ts) performs one linear walk that skips the host language's quoted strings and comments (per-engine [`Delimiters`](../packages/preprocessor-api/src/context-utils.ts)), so neither the `EXEC <prefix>` anchor nor the terminating `;` can be matched inside them; host strings follow the host lexer's semantics (they may span lines), while an engine's own statement bodies follow the embedded grammar's (they end at a line break). Every `EXEC <other> ...;` statement is skipped as opaque text (the host tokenizer treats any `EXEC` statement as one token, so the two views stay aligned); between statements the walk records the engine's secondary anchor (`SQL TYPE IS`, `DFHRESP(`/`DFHVALUE(`) and tracks the block structure (`PROC`, `DO`/`BEGIN`/`SELECT`, `END` - including labeled `END`s closing several blocks at once) into enclosing-procedure checkpoints, which feed `findEnclosingProcedureEnd` for the per-procedure declarations. A statement that never closes is emitted as an unterminated fragment so it still gets parsed and diagnosed.
...
- **Replacement**: an ordinary statement is replaced with the no-op `DO; END;` (a placeholder the engines are free to change, even to nothing); its identifier tokens (host variables, each name part of a qualified `:A.B` separately) become ordinary linkable references emitted via `PhaseResult.references`. They have no AST parent: at link time `resolveReference` adopts the element parsed from the replacement text itself (see `Reference.anchor`: the annotate pass captures the first token lexed from the edit's replacement span, whatever the text - no assumption about the host language's tokens - and the parser assigns that token its element). This is exact per inclusion of a copybook, across include boundaries, and under `RULES(MULTICLOSE)`, because the anchor rides object identity through the pipeline rather than text offsets. An edit whose replacement is empty (or lexes to nothing) gets no anchor, and its references fall back to adopting a parsed statement surrounding the reference's source position (the following statement, or the preceding one inside the block when the following one is the block's `END`). `EXEC SQL INCLUDE member` instead resolves the member (via `include`, taking the same lib-resolution path as `%INCLUDE`): the host blanks the statement, records the splice of the included file's text as foreign segments, and hands the engine a fresh, *unprocessed* context over that text, which the engine processes recursively (so a DCLGEN copybook's declarations can still find the including file's procedure); the nested context is built lazily, when the including context is. The statement becomes a regular `IncludeDirective` in the preprocessor AST, riding the same hover/definition/validation paths as `%INCLUDE`.
- **Built-ins**: constructs that appear *outside* `EXEC` statements are handled by the engines on the same walk. DB2: `SQL TYPE IS ...` attribute declarations become their PL/I equivalent types ([`sql-type.ts`](../packages/preprocessor-db2/src/engine/sql-type.ts) mirrors the PL/I parser's recovery: only the consumed tokens are replaced, IBM375x/378x diagnostics carry the compiler's severity suffix), with the `SQL_LOB*` declaration blocks inserted once per enclosing procedure. CICS: `DFHRESP(condition)` and `DFHVALUE(cvda)` become their numeric values ([`cics-values.ts`](../packages/preprocessor-cics/src/engine/cics-values.ts), generated from IBM's EIBRESP and CVDA tables), and every procedure containing an `EXEC CICS` statement gets the `DFH*` runtime declarations (`DFHEIBLK` with `EIBRESP` etc.) inserted once, right after the procedure's `;`. Declarations are zero-width `replace`s; at the same offset they sort before a consuming edit.
- **Metadata**: `collectExecMetadata` in [exec-phase.ts](../packages/language/src/preprocessor/exec-phase.ts) turns the recorded edits (of the entry context and, recursively, of every included one) into the LSP-facing artifacts - per-token semantic-highlighting tokens in host coordinates (registered via `directiveTokens`, plus `string`-typed tokens for the two leading words of an `EXEC` statement), and one `Reference` per host-variable name part. Tokens whose text was itself macro-generated have no host position and are deliberately left unregistered. Included files are registered token-less; `mergeForeignTokens` fills their registration from the final stream.
...
`EXEC SQL INCLUDE` resolves through the same machinery via `PreprocessorContext.include`.
